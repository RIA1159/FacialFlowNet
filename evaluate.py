...
@torch.no_grad()
def validate_facialflow(model, iters = 32):
    """ Peform validation using the EXP (train) split """
    model.eval()
    val_dataset = datasets.FacialFlow(split = "test")
    ...
    for val_id in range(len(val_dataset)):
        image1, image2, flow_gt_e, flow_gt_h, _, _, mask= val_dataset[val_id]
        image1 = image1[None].cuda()
        image2 = image2[None].cuda()

        padder = InputPadder(image1.shape, mode = "exp")
        image1, image2 = padder.pad(image1, image2)
        flow_gt = flow_gt_e



        flow_low, flow_pr, head_flow, exp_flow = model(image1, image2, iters=iters, test_mode=True)
        flow = padder.unpad(flow_pr[0]).cpu()

        mask = mask.cpu().numpy()
        pixel_amount = np.sum(mask)


        epe = torch.sum((flow - flow_gt)**2, dim=0).sqrt()
        epe = epe.view(-1).numpy()

        epe_list.append(np.sum(epe) / pixel_amount)
        px1_list.append(np.sum(epe < 1) / pixel_amount)
        px3_list.append(np.sum(epe < 3) / pixel_amount)
        px5_list.append(np.sum(epe < 5) / pixel_amount)

        print("\r validation: {}/{}  epe: {}".format(val_id,len(val_dataset),epe_list[-1]), end=" ")



    epe = np.mean(np.array(epe_list))
    px1 = np.mean(np.array(px1_list))
    px3 = np.mean(np.array(px3_list))
    px5 = np.mean(np.array(px5_list))

    print("\nValidation  EPE: %f, 1px: %f, 3px: %f, 5px: %f" % ( epe, px1, px3, px5))

    output_file = os.path.join("./evaluate_result.npy") 


    if not os.path.exists(output_file):
        results = []
    else:
        results = np.load(output_file, allow_pickle = True).tolist()
    
    result = {"model": args.model.split("/")[-1], "epe":epe, "px1":px1, "px3":px3, "px5":px5}
    # result = {"epe":epe, "px1":px1, "px3":px3, "px5":px5}
    print(result)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if args.dataset == 'chairs':
            ...
            validate_facialflow(model.module, iters=args.iters)
