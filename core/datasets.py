...
class FlowDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # mask2 = np.load(self.mask_list[index][1]).astype(np.uint8)
        ...
        if self.occ_list is not None:
            ...
            return img1, img2, exp_flow, head_flow, valid_e.float(), valid_h.float(), mask #, self.extra_info[index]
...
class FacialFlow(FlowDataset):
    def __init__(self, aug_params=None, split = "train", dataset_root = "datasets/FacialFlowNet"):
        super(FacialFlow, self).__init__(aug_params)
        ...
        mask_root = osp.join(dataset_root, "mask", split)
